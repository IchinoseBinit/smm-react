...
>(
  (
    ...
    async function transcodeToSingleAspectRatio(file: File): Promise<string> {
      if (!ffmpeg) {
        ffmpeg = new FFmpeg()
        await ffmpeg.load()
      }

      // Automatically determine aspect ratio based on surface type
      const aspectRatio = surfaceType[0] === "POST" ? "1:1" : "9:16"
      ...
      try {
        ...
        console.log(
          `Auto-converting to ${aspectRatio} for ${surfaceType[0]} surface type (${width}x${height})`
        )
        ...
        console.log(
          `Auto-conversion completed: ${aspectRatio} for ${surfaceType[0]}`
        )
    ...
    const handleAspectRatioClick = useCallback(
      async (aspectRatio: string) => {
        ...
        try {
          ...
          console.log(`Converting video for ${surfaceType[0]} surface:`, {
          ...
          // Now we only pass the file, aspect ratio is determined automatically
          const url = await transcodeToSingleAspectRatio(currentVideoFile)
          const duration = ((performance.now() - startTime) / 1000).toFixed(1)

          console.log(`Auto-converted for ${surfaceType[0]} - URL:`, url)
    ...
    // Add a function to handle auto-conversion when scheduling
    const handleAutoConversionForScheduling = useCallback(async () => {
      if (!currentVideoFile) {
        console.log("No video file available for auto-conversion")
        return null
      }

      const targetAspectRatio = surfaceType[0] === "POST" ? "1:1" : "9:16"

      // Check if already converted to target ratio
      if (videoUrls[targetAspectRatio]) {
        console.log(`Video already converted to ${targetAspectRatio}`)
        setSelectedAspectRatio(targetAspectRatio)
        return videoUrls[targetAspectRatio]
      }

      try {
        console.log(
          `🔄 Auto-converting video for scheduled ${surfaceType[0]} post to ${targetAspectRatio}`
        )

        // Call the transcoding function (silent - no toasts)
        const url = await transcodeToSingleAspectRatio(currentVideoFile)

        // Update state with converted video
        setVideoUrls((prev) => ({ ...prev, [targetAspectRatio]: url }))
        setSelectedAspectRatio(targetAspectRatio)

        // Reload video elements to show converted version
        setTimeout(() => {
          const videoElements = document.querySelectorAll("video")
        ...
        console.log(
          `✅ Auto-conversion completed silently for ${surfaceType[0]} - ${targetAspectRatio}`
        )
        return url
      } catch (error) {
        console.error("❌ Silent auto-conversion failed:", error)
        return null
      }
    }, [currentVideoFile, videoUrls, surfaceType, transcodeToSingleAspectRatio])

    // Expose the auto-conversion function to parent component
    useImperativeHandle(ref, () => ({
      triggerAutoConversion: handleAutoConversionForScheduling,
    }))
